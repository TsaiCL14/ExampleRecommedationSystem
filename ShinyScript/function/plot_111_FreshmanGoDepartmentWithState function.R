plot_111_FreshmanGoDepartmentWithState <- function(DEPT,UNIV,STATE){
  ...
  for(i in 1:dim(plotData)[1]){
    ...
    if(grepl("A",storage$V10[which(storage$V8 == "unit_1" & storage$V9 == DEPT)])){ plotData$V12[i] <- "Accept"}
    if(grepl("W",storage$V10[which(storage$V8 == "unit_1" & storage$V9 == DEPT)])){ plotData$V12[i] <- "Wait"}
    if(!grepl("A|W",storage$V10[which(storage$V8 == "unit_1" & storage$V9 == DEPT)])){ plotData$V12[i] <- "N_Accept"}
  }
